# https://github.com/yzd-v/cls_KD/blob/1.0/mmcls/models/dis_losses/nkd.py
import torch
...
class NKDLoss(nn.Module):
    ...
    def forward(self, logit_s, logit_t, gt_label):
        ...
        C = logit_s.shape[1]
        logit_s = logit_s.permute(0, 2, 3, 4, 1).contiguous().view(-1, C)
        logit_t = logit_t.permute(0, 2, 3, 4, 1).contiguous().view(-1, C)
        label = gt_label.contiguous().view(-1).long().unsqueeze(1)
