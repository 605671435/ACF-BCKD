...
def find_best_in_multi_run(checkpoint):
    dirs = [dir for dir in os.listdir(checkpoint) if dir.startswith('run')]
    dirs = [osp.join(checkpoint, dir) for dir in dirs]
...
def main(args):
    ...
    checkpoint1 = find_best_in_multi_run(args.checkpoint1)
    checkpoint2 = find_best_in_multi_run(args.checkpoint2)

    model1 = init_model(args.config1, checkpoint1, device=args.device)
    # init visualizer
    visualizer = VISUALIZERS.build(model1.cfg.visualizer)
    visualizer.draw_featmap = modify

    model2 = init_model(args.config2, checkpoint2, device=args.device)
    ...
    with recorder_manager1:
        # test a single image
        _ = inference_model(model1, args.img, args.img.replace('img_dir', 'ann_dir').replace('jpg', 'png'))

    with recorder_manager2:
        # test a single image
        _ = inference_model(model2, args.img, args.img.replace('img_dir', 'ann_dir').replace('jpg', 'png'))
    ...
    for name1, name2 in zip(mappings1.keys(), mappings2.keys()):
        ...
        data_idx = getattr(record1, 'data_idx') if hasattr(record1, 'data_idx') else None
        ...
        data_idx = getattr(record2, 'data_idx') if hasattr(record2, 'data_idx') else None
        ...
        for i, (feat1, feat2) in enumerate(zip(feats1, feats2)):
            ...
            drawn_img = visualizer.draw_featmap(
                ...
                alpha=args.alpha)
            mmcv.imwrite(drawn_img, osp.join('./out_dir', f'model1_{name1}_model2_{name2}_{i}.jpg'))
            # visualizer.add_datasample(
            #     name=f'model1_{name1}_model2_{name2}_{i}',
            #     image=drawn_img,
            #     out_file=osp.join('./out_dir/gt', 'diff.jpg'))
