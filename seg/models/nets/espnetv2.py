...
from .utils.espnet_utils import *
from .utils.efficient_pyramid_pool import EfficientPyrPool
from .utils.efficient_pt import EfficientPWConv
from .utils.espnetv2 import EESPNet
...
class ESPNetv2Segmentation(nn.Module):
    ...
    def __init__(self, args, classes=21, dataset='pascal'):
        ...
        self.bu_br_l2 = nn.Sequential(nn.BatchNorm3d(dec_planes[0]),
                                      nn.PReLU(dec_planes[0])
                                      )
        self.bu_br_l3 = nn.Sequential(nn.BatchNorm3d(dec_planes[1]),
                                      nn.PReLU(dec_planes[1])
                                      )
        self.bu_br_l4 = nn.Sequential(nn.BatchNorm3d(dec_planes[2]),
    ...
    def get_basenet_params(self):
        ...
        for i in range(len(modules_base)):
            for m in modules_base[i].named_modules():
                if isinstance(m[1], nn.Conv3d) or isinstance(m[1], nn.BatchNorm3d) or isinstance(m[1], nn.PReLU):
    ...
    def forward(self, x):
        ...
        '''
        x_size = (x.size(2), x.size(3), x.size(4))
        ...
        return F.interpolate(bu_out, size=x_size, mode='trilinear', align_corners=True)
