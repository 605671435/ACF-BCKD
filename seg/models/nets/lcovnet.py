...
class UnetBlock_Encode_BottleNeck(nn.Module):
    def __init__(self, in_channels, out_channel):
        ...
        self.conv2_1 = nn.Sequential(
            nn.Conv3d(self.out_chns, self.out_chns, kernel_size=(3, 3, 1),
                      padding=(1, 1, 0), groups=self.out_chns, bias=False),
            nn.BatchNorm3d(self.out_chns),
            nn.ReLU6(inplace=True),
            # nn.Dropout(p=0.2)
        )
...
class LCOV_Net(nn.Module):
    def __init__(self, C_in=32, n_classes=17, m=1, is_ds=True):
        ...
        self.segout1 = nn.Conv3d(
            self.ft_chns[3], self.n_class, kernel_size=1, padding=0, bias=False)
        ...
        self.segout2 = nn.Conv3d(
            self.ft_chns[2], self.n_class, kernel_size=1, padding=0, bias=False)
        ...
        self.segout3 = nn.Conv3d(
            self.ft_chns[1], self.n_class, kernel_size=1, padding=0, bias=False)
        ...
        self.segout4 = nn.Conv3d(
            self.ft_chns[0], self.n_class, kernel_size=1, padding=0, bias=False)
