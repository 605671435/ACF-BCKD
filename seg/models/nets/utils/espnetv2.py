...
from .eesp import DownSampler, EESP
from .espnet_utils import CBR
from torch.nn import init
import torch.nn.functional as F
from . import espnetv2_config as config_all
...
class EESPNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.level2_0 = DownSampler(
            out_channel_map[0], out_channel_map[1], k=K[0],
            r_lim=recept_limit[0], reinf=self.input_reinforcement, inp_reinf=channels_in)  # out = 56

        self.level3_0 = DownSampler(
            out_channel_map[1], out_channel_map[2], k=K[1],
            r_lim=recept_limit[1], reinf=self.input_reinforcement, inp_reinf=channels_in) # out = 28
        ...
        self.level4_0 = DownSampler(
            out_channel_map[2], out_channel_map[3], k=K[2],
            r_lim=recept_limit[2], reinf=self.input_reinforcement, inp_reinf=channels_in) #out = 14
    ...
    def forward(self, input, p=0.2):
        ...
        output_g = F.adaptive_avg_pool3d(out_l5, output_size=1)
