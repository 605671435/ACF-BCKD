...
class CBR(nn.Module):
    ...
    def __init__(self, nIn, nOut, kSize, stride=1, dilation=1, groups=1, act_name='prelu'):
        ...
        self.cbr = nn.Sequential(
            nn.Conv3d(nIn, nOut, kSize, stride=stride, padding=padding, bias=False, groups=groups, dilation=dilation),
            nn.BatchNorm3d(nOut),
...
class CB(nn.Module):
    ...
    def __init__(self, nIn, nOut, kSize, stride=1, dilation=1, groups=1):
        ...
        self.cb = nn.Sequential(
            nn.Conv3d(nIn, nOut, kSize, stride=stride, padding=padding, bias=False, groups=groups, dilation=1),
            nn.BatchNorm3d(nOut),
        )
...
class Shuffle(nn.Module):
    ...
    def forward(self, x):
        batchsize, num_channels, height, width, dim = x.data.size()
        channels_per_group = num_channels // self.groups
        x = x.view(batchsize, self.groups, channels_per_group, height, width, dim)
        x = torch.transpose(x, 1, 2).contiguous()
        x = x.view(batchsize, -1, height, width, dim)
        return x
...
class DWConv(nn.Module):
    def __init__(self, nin):
        ...
        self.dw_layer = nn.Sequential(
            nn.Conv3d(nin, nin, kernel_size=3, stride=1, padding=1, bias=False, groups=nin),
            nn.BatchNorm3d(nin),
            nn.PReLU(nin)
        )
