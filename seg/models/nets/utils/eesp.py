...
from .espnet_utils import *
import math
import torch
from . import espnetv2_config as config
...
class DownSampler(nn.Module):
    ...
    def __init__(self, nin, nout, k=4, r_lim=9, reinf=True, inp_reinf=config_inp_reinf):
        '''
            :param nin: number of input channels
            :param nout: number of output channels
            :param k: # of parallel branches
        ...
        self.avg = nn.AvgPool3d(kernel_size=3, padding=1, stride=2)
        if reinf:
            self.inp_reinf = nn.Sequential(
                CBR(inp_reinf, inp_reinf, 3, 1),
                CB(inp_reinf, nout, 1, 1)
            )
    ...
    def forward(self, input, input2=None):
        ...
        if input2 is not None:
            ...
            w1 = avg_out.size(2)
            while True:
                input2 = F.avg_pool3d(input2, kernel_size=3, padding=1, stride=2)
                w2 = input2.size(2)
                if w2 == w1:
                    break
