...
# Date: July 2022
# copy right from:
# https://github.com/lafith/Mobile-UNet/blob/main/model.py
    ...
    def __init__(self, in_c, out_c, stride, expansion_factor=6, deconvolve=False):
        ...
        if deconvolve:
            self.conv = nn.Sequential(
                ...
                nn.Conv3d(self.in_c, ex_c, 1, 1, 0, bias=False),
                nn.BatchNorm3d(ex_c),
                nn.ReLU6(inplace=True),
                # depthwise convolution
                nn.ConvTranspose3d(ex_c, ex_c, 4, self.stride, 1, groups=ex_c, bias=False),
                nn.BatchNorm3d(ex_c),
                nn.ReLU6(inplace=True),
                # pointwise convolution
                nn.Conv3d(ex_c, self.out_c, 1, 1, 0, bias=False),
            ...
            self.conv = nn.Sequential(
                ...
                nn.ReLU6(inplace=True),
                # depthwise convolution
                nn.Conv3d(ex_c, ex_c, 3, self.stride, 1, groups=ex_c, bias=False),
                nn.BatchNorm3d(ex_c),
                nn.ReLU6(inplace=True),
                # pointwise convolution
                nn.Conv3d(ex_c, self.out_c, 1, 1, 0, bias=False),
        ...
        self.conv1x1 = nn.Conv3d(self.in_c, self.out_c, 1, 1, 0, bias=False)
...
class MobileUNet(nn.Module):
    ...
    def __init__(self, in_channels, num_classes):
        ...
        self.conv1x1_encode = nn.Conv3d(320, 1280, kernel_size=1, stride=1)
        # decoding arm
        ...
        self.DConv4x4 = nn.ConvTranspose3d(16, 16, 4, 2, 1, groups=16, bias=False)
        # Final layer: output channel number can be changed as per the usecase
        self.conv1x1_decode = nn.Conv3d(16, num_classes, kernel_size=1, stride=1)
    ...
    def depthwise_conv(self, in_c, out_c, k=3, s=1, p=0):
        ...
        """
        conv = nn.Sequential(
            nn.Conv3d(in_c, in_c, kernel_size=k, padding=p, groups=in_c, stride=s),
            nn.BatchNorm3d(num_features=in_c),
            nn.ReLU6(inplace=True),
