...
from mmcv.cnn import ConvModule, build_activation_layer, build_plugin_layer
...
class ShuffleNetV1(BaseBackbone):
    ...
    def __init__(self,
        ...
        for i, num_blocks in enumerate(self.stage_blocks):
            ...
            layer = self.make_layer(channels[i], num_blocks, strides[i], first_block)
    ...
    def make_layer(self, out_channels, num_blocks, stride, first_block=False):
...
@MODELS.register_module()
class ShuffleNetV1_KD(ShuffleNetV1):
    def __init__(self,
                 stage_plugin=dict(type='EX_KD'),
                 out_channels=[0, 480, 0],
                 **kwargs):
        super().__init__(**kwargs)
        plugin = stage_plugin.copy()
        for i, out_index in enumerate(self.out_indices):
            if out_channels[i] == 0:
                continue
            layer = self.layers[out_index]
