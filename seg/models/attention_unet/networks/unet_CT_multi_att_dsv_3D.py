...
from ..layers.grid_attention_layer import GridAttentionBlock3D, GridAttentionBlock2D
...
class MultiAttentionBlock2D(nn.Module):
    def __init__(self,
        ...
        if gate_size is None:
            gate_size = 2 * in_channels
        if inter_size is None:
            inter_size = in_channels
        self.gate_block_1 = GridAttentionBlock2D(in_channels=in_channels, gating_channels=gate_size,
                                                 inter_channels=inter_size, mode=nonlocal_mode,
                                                 sub_sample_factor= sub_sample_factor)
        self.gate_block_2 = GridAttentionBlock2D(in_channels=in_channels, gating_channels=gate_size,
                                                 inter_channels=inter_size, mode=nonlocal_mode,
                                                 sub_sample_factor=sub_sample_factor)
        self.combine_gates = nn.Sequential(nn.Conv2d(in_channels * 2, in_channels, kernel_size=1, stride=1, padding=0),
    ...
    def forward(self, input, gating_signal):
        ...
        return self.combine_gates(torch.cat([gate_1, gate_2], 1))

class SingleAttentionBlock2D(nn.Module):
    def __init__(self,
                 in_channels,
                 gate_size=None,
                 inter_size=None,
                 nonlocal_mode='concatenation',
                 sub_sample_factor=(2, 2)):
        super(SingleAttentionBlock2D, self).__init__()
        if gate_size is None:
            gate_size = 2 * in_channels
        if inter_size is None:
            inter_size = in_channels
        self.gate_block_1 = GridAttentionBlock2D(in_channels=in_channels, gating_channels=gate_size,
                                                 inter_channels=inter_size, mode=nonlocal_mode,
                                                 sub_sample_factor= sub_sample_factor)
        self.combine_gates = nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=1, stride=1, padding=0),
                                           nn.BatchNorm2d(in_channels),
                                           nn.ReLU(inplace=True)
                                           )
